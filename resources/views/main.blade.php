...
<header class="header-buttons">
  <nav>
    <a href="https://www.utmn.ru/" style="margin-left: 14px; margin-right: 14px;">
    ...
    <div >
        @if(Auth::check())
            <a href="{{ route('lk') }}" style="margin-left: 14px; margin-right: 14px;">Личный кабинет</a>
            <a href="#" style="margin-left: 14px; margin-right: 14px;" 
                onclick="event.preventDefault(); document.getElementById('logoutForm').submit();">Выход</a>
            <form method="POST" action="{{ route('logout') }}" id="logoutForm" style="display: none;">
                @csrf
            </form>
        @else
            <a href="{{ route('login') }}" style="margin-left: 14px; margin-right: 14px;">Вход</a>
            <a href="{{ route('register') }}" style="margin-left: 14px; margin-right: 14px;">Регистрация</a>
        @endif
    </div>
  </nav>
</header>
...
<div class="centrator">
  ...
  @foreach ($confs as $conf)
    <section id='{{ $conf->id }}' style="background-color: rgb(4, 159, 217, 0.5)" class="program-section section-hoverable font-size">
