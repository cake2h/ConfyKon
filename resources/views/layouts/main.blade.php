        ...
        <div class="menu">
            @auth
                @if (Auth::user()->isAdmin())
                    <a class="menu__button" href="{{ route('admin.index') }}">Админ-панель</a>
                @endif
                <a class="menu__button" href="{{ route('dashboard.index') }}">Личный кабинет</a>
                <form method="POST" action="{{ route('logout') }}" id="logoutForm">
                    @csrf
                    <button class="menu__button" type="submit">Выйти из аккаунта</button>
                </form>
            @else
                <a class="menu__button" href="{{ route('login') }}">Вход</a>
                <a class="menu__button" href="{{ route('register') }}">Регистрация</a>
            @endauth
        </div>
    </header>
